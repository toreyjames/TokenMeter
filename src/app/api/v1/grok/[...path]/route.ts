...
export async function POST(
  ...
  try {
    ...
    if (!keyData.grokKey) {
      return NextResponse.json(
        { error: 'No Grok API key configured. Add one in your TokenMeter dashboard.' },
        { status: 400 }
      );
    }
    ...
    // Forward to Grok (OpenAI-compatible)
    const grokResponse = await fetch(`${GROK_BASE_URL}${endpoint}`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${keyData.grokKey}`,
    ...
    // Calculate cost
    const costCents = calculateCost('grok', model, { 
      input: inputTokens, 
      output: outputTokens 
    });
